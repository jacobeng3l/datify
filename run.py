...
def library():
    ...
    if "items" in request.form:
        for song_id in request.form["items"].split(','):
            ...
            sql = "update plays set next_song_id={song_id} where current_song_id=(select current_song_id from plays p inner join user u on p.user_id=u.user_id where u.user_id={user_id} order by times desc limit 1)".format(song_id=song_id, user_id=session['user_id'])
            sql_execute(sql)
